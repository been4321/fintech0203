	...
	<nav class="navbar navbar-inverse">
		<div class="container-fluid">
			...
			<ul class="nav navbar-nav ml-auto">
				...
				<li class="dropdown"><a class="btn-lg dropdown-toggle"
				...
				<li class="dropdown"><a class="btn-lg dropdown-toggle"
					...
					<ul class="dropdown-menu">
						<li class="list-group-item"><a
							href="<c:url value="/shopMain.jsp"/>">상품목록</a></li>
					</ul></li>
			...
			<ul class="nav navbar-nav navbar-right">
				<c:choose>
					<c:when test="${empty sessionId}">
						<li><a class="btn-lg dropdown-toggle"
							href="<c:url value="loginMember.1jsp"/>"><span
								class="glyphicon glyphicon-user"></span>로그인</a></li>
						<li><a class="btn-lg dropdown-toggle"
							href="<c:url value="addMember.jsp"/>"><span
					...
					<c:when test="${sessionId == 'admin'}">
						<li style="padding-top: 8px; color: white; font-size: 22px">
							<c:forEach var="row" items="${result.rows}">[<c:out
									value="${row.name}" />
							</c:forEach>님]
						</li>
						...
						<li class="dropdown"><a class="btn-lg dropdown-toggle"
							data-toggle="dropdown" href="#">관리자용</a>
							<ul class="dropdown-menu">
								<li class="list-group-item"><a
									href="<c:url value="//shopMain.jsp"/>">상품목록</a></li>
								<li class="list-group-item"><a
					...
					<c:when test="${sessionId != 'admin'}">
						<li style="padding-top: 8px; color: white; font-size: 22px"><c:forEach
								var="row" items="${result.rows}">[<c:out
									value="${row.name}" />
							</c:forEach>님]</li>
						<li style="font-weight: bold" class="nav-item"><a
							class="btn-lg nav-link" href="<c:url value="logoutMember.jsp"/>">로그아웃
						</a></li>
						<li style="font-weight: bold" class="nav-item"><a
